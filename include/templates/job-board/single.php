...
<div class="wpjb wpjb-job wpjb-page-single">
	...
	elseif ( $checkCapacities->checkUsersCapacities() === false ):
		...
		?>
		<div class="wpjb-flash-error wpjb-flash-small">
			<span class="wpjb-glyphs wpjb-icon-attention">Lo sentimos, para aplicar a cualquier oferta laboral debe completar su perfil</span>
		</div>
			...
			<div class="wpjb-job-buttons">
				<?php if ( ! wpjb_conf( "front_hide_apply_link" ) ): ?>
					<?php if ( $application_url ): ?>
						<a class="wpjb-button"
						   href="<?php esc_attr_e( $application_url ) ?>"><?php _e( "Apply", "wpjobboard" ) ?></a>
					<?php else: ?>
						<a class="wpjb-button wpjb-form-toggle wpjb-form-job-apply"
						   href="<?php esc_attr_e( wpjb_link_to( "job", $job, [ "form" => "apply" ] ) ) ?>#wpjb-scroll"
						   rel="nofollow"
						   data-wpjb-form="wpjb-form-job-apply"><?php _e( "Apply Online", "wpjobboard" ) ?> <span
				...
				<?php foreach ( $relatedJobs->job as $relatedJob ): ?>
					...
					<div class="wpjb-grid-row <?php wpjb_job_features( $relatedJob ); ?>">
						<div class="wpjb-grid-col wpjb-col-70">
							...
							<?php endif; ?>
							<?php if ( $relatedJob->isNew() ): ?><span
									class="wpjb-bulb"><?php _e( "new", "wpjobboard" ) ?></span><?php endif; ?>
						</div>
